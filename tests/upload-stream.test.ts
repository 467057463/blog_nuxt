...
// 这里验证 OSS key 拼接、文件名校验、multipartUpload 被以 stream 形式调用（mock client，不真连 OSS）。

// 用依赖注入构造一个可测的 uploadStream，与 server/utils/oss.ts 中实现同构：
// 传入一个假 client，断言其被正确调用、key 正确、stream 原样透传。
function makeUploadStream (client: { multipartUpload: (key: string, stream: NodeJS.ReadableStream, options?: object) => Promise<void> }) {
  return async function uploadStream ({ directory, filename, stream }: {
    ...
    try {
      ...
      await client.multipartUpload(key, stream, { partSize: 1024 * 1024 * 5 })
      return key
    } catch (error) {
      console.error(error)
      return null
...
test('uploadStream 用原始文件名拼接 key 并将 stream 透传给 multipartUpload', async () => {
  const captured: { key: string; stream: NodeJS.ReadableStream }[] = []
  const client = {
    async multipartUpload (key: string, stream: NodeJS.ReadableStream) {
  ...
  assert.equal(captured.length, 1, 'multipartUpload 应被调用一次')
